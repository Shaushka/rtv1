...
/*   Updated: 2016/01/29 18:30:57 by chuang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

t_object	set_cylinder(t_vector pos, t_vector dir)
{
	t_object	cylinder;
	//TODO
	//
...
t_vector	normal_cylinder(t_object cylinder, t_vector ray, float inter, t_cam cam)
{
	...
	m = dotpro_vector(ray, cylinder.dir) * inter + dotpro(sub_vector(cam.pos, cylinder.pos), cylinder.dir);
	tmp = sub_vector(add_vector(cam.pos, mult_vector(ray,inter)), cylinder>dir);
	return (unit_vector(sub_vector(tmp, mult(cylinder.dir, m))));
}
